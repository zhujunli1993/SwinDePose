...
def render_mulimage_angles(obj_dir, data_id):
    root = os.path.join(obj_dir.split('/')[0],obj_dir.split('/')[1])
    scene = obj_dir.split('/')[2]
   
    anglesfileX = os.path.join(root, scene, data_id+ "_anglesX.txt")
    anglesfileY = os.path.join(root, scene, data_id+ "_anglesY.txt")
    anglesfileZ = os.path.join(root, scene, data_id+ "_anglesZ.txt")

    new_img_file_angles = os.path.join(obj_dir, data_id+ ".png")
...
def render_mulimage_signed(obj_dir, data_id):
    root = os.path.join(obj_dir.split('/')[0],obj_dir.split('/')[1])
    scene = obj_dir.split('/')[2]
    signfileX = os.path.join(root, scene, data_id+ "_signed_anglesX.txt")
    signfileY = os.path.join(root, scene, data_id+ "_signed_anglesY.txt")
    signfileZ = os.path.join(root, scene, data_id+ "_signed_anglesZ.txt")

    new_img_file_signed = os.path.join(obj_dir, data_id+ ".png")
...
def render_mulimage_angles_pkl(obj_dir, data_id):
    root = os.path.join(obj_dir.split('/')[0],obj_dir.split('/')[1])
    scene = obj_dir.split('/')[2]
...
def render_mulimage_signed_pkl(obj_dir, data_id):
    root = os.path.join(obj_dir.split('/')[0],obj_dir.split('/')[1])
    scene = obj_dir.split('/')[2]
    signfileX = os.path.join(root, scene, data_id+ "_signed_anglesX.txt")
    signfileY = os.path.join(root, scene, data_id+ "_signed_anglesY.txt")
