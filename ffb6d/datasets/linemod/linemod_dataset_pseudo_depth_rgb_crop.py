...
class Dataset():
    ...
    def add_real_back(self, rgb, rgb_s, rgb_ori, labels, dpt, dpt_msk):
        ...
        with Image.open(os.path.join(self.cls_root, "rgb", real_item+'.png')) as ro:
            back_r = np.array(ro)[:, :, :3] * bk_label[:, :, None]
        ...
        if self.rng.rand() < 0.6:
            ...
            rgb_ori = rgb_ori * (msk_back == 0).astype(rgb_ori.dtype) + back_r * msk_back
    ...
    def get_crop(self, bbox_folder, item_name, max_w=168, max_h=190):
        bbox = np.load(os.path.join(bbox_folder, 'bbox.npy'))
        boolArr = (bbox[:,0] == item_name)
        result = np.where(boolArr)
        index = int(result[0])
        loc = bbox[index]
        l_w, l_h, h_w, h_h = int(loc[1]), int(loc[2]), int(loc[3]), int(loc[4])
        
        return l_w, l_h, h_w, h_h
    def get_item(self, item_name):
        ...
        if ".npz" in item_name:
            ...
            #valid_dpt_mm = np.ma.masked_array(dpt_mm, valid_msk)
                if self.opt.crop:
                    l_w, l_h, h_w, h_h = self.get_crop(os.path.join(self.cls_root, 'mask'), item_name, self.opt.max_w, self.opt.max_h)
                    
                    rgb = rgb[l_h:h_h,l_w:h_w, :]
                    rgb_s = rgb_s[l_h:h_h,l_w:h_w, :]
                    rgb_ori = rgb_ori[l_h:h_h,l_w:h_w, :]
                    labels = labels[l_h:h_h,l_w:h_w, :]
                    # dpt_mm = dpt_mm[l_h:h_h,l_w:h_w, :]
            ...
            dpt_mm_rgb[index] = 255
            if self.opt.crop:
                dpt_mm_rgb = dpt_mm_rgb[l_h:h_h,l_w:h_w]
        
        cam_scale = 1000.0
        
        dpt_mm = dpt_mm.copy().astype(np.uint16)
        if self.opt.crop:
            dpt_mm_crop = dpt_mm[l_h:h_h,l_w:h_w]
        nrm_map = normalSpeed.depth_normal(
            dpt_mm_crop, K[0][0], K[1][1], 5, 2000, 20, False
        )
        ...
        if self.opt.crop:
            dpt_xyz = dpt_xyz[l_h:h_h,l_w:h_w,:]
        ...
        if self.add_noise and rnd_typ != 'real':
            if rnd_typ == 'render' or self.rng.rand() < 0.8:
                rgb_ori = self.rgb_add_noise(rgb_ori)
                rgb = self.rgb_add_noise(rgb)
                rgb_s = self.rgb_add_noise(rgb_s)
                rgb_labels = labels.copy()
                msk_dp = dpt_mm > 1e-6
                rgb, rgb_s, rgb_ori, dpt_mm = self.add_real_back(rgb, rgb_s, rgb_ori, rgb_labels, dpt_mm, msk_dp)
        ...
        rgb_ori = np.transpose(rgb_ori, (2, 0, 1)) # hwc2chw
