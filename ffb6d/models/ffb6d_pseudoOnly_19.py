...
class FFB6D(nn.Module):
    def __init__(
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=(self.up_rndla_oc[-1]+self.up_rgb_oc[-1]), nhead=1)    
        self.fuse_final = nn.TransformerEncoder(self.encoder_layer, num_layers=1)
        self.rgbd_seg_layer = (
    ...
    def forward(
        ...
        rgbd_emb = rgbd_emb.permute(0,2,1)
        
        rgbd_emb = self.fuse_final(rgbd_emb)
        rgbd_emb = rgbd_emb.permute(0,2,1)
        # ###################### prediction stages #############################
