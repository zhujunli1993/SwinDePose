...
class FFB6D(nn.Module):
    def __init__(
        ...
        self.n_pts = n_pts
        self.n_kps = n_kps
        self.swin_ffb = swin.SwinTransformer(in_channels=4)
    ...
    def forward(
        ...
        input_img = torch.cat((inputs['nrm_angles'],inputs['dpt_map_m'].unsqueeze(dim=1)),dim=1)
        
        feat_nrm = self.swin_ffb(input_img) # [1,96,120,160]->[1,192,60,80]->[1,384,30,40]->[1,768,15,20]
