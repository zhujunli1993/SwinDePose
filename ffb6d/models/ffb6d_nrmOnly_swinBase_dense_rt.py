...
from models.cnn.pspnet_swin import PSPNet
...
class Linear(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fcn1 = nn.Conv1d(in_channels, 512, kernel_size=1).cuda()

        self.fcn_r = nn.Linear(512, 3).cuda()
        # self.fcn_r = nn.Conv1d(512, 3, kernel_size=1)
        self.fcn_xy = nn.Linear(512, 2).cuda()
        # self.fcn_xy = nn.Conv1d(512, 2, kernel_size=1)
        self.fcn_z = nn.Linear(512, 1).cuda()
        # self.fcn_z = nn.Conv1d(512, 1, kernel_size=1)
    ...
    def forward(self, features):
        ...
        # x = features.type(torch.FloatTensor)
        
        x = self.dropout(self.relu(self.fcn1(features)))
...
class FFB6D(nn.Module):
    def __init__(
        ...
        self.n_pts = n_pts
        self.n_kps = n_kps
        self.swin_ffb = swin.SwinTransformer(embed_dims=128,depths=[2, 2, 18, 2],num_heads=[4, 8, 16, 32])
        self.psp_head = uper_head.UPerHead(in_channels=[128, 256, 512, 1024],channels=512,in_index=[0,1,2,3],num_classes=2)
        # self.psp_fuse_head = uper_head.UPerHead(in_channels=[192,384,768,1536],channels=256,in_index=[0,1,2,3],num_classes=2)
        # self.psp_head = psp_models['resnet34'.lower()]()
    ...
    def forward(
        ...
        bs, _, h_, w_ = feat_nrm[-1].shape
        feat_nrm_final = feat_nrm[-1].view(bs, _, h_*w_)
        ...
        for i_ds in range(4):
            ...
            p_emb0 = self.random_sample(f_encoder_i, inputs['cld_sub_idx%d' % i_ds]) # [1, 64, 4800, 1] -> [1, 128, 1200, 1] ->[1, 256, 300, 1] 
            if i_ds == 0:
                ds_pc_emb.append(f_encoder_i)
            p_emb = p_emb0
            ds_pc_emb.append(p_emb)
        # ds_pc_emb: [1, 64, 19200, 1] -> [1, 64, 4800, 1] -> [1, 128, 1200, 1] -> [1, 256, 300, 1] 
        
        # ###################### decoding stages for semantic segmentation #############################
        ...
        # feat_nrm_seg = intep(feat_up_nrm)
        # feat_nrm_seg = feat_nrm_seg.view(bs, di, -1)
        # choose_emb = inputs['choose'].repeat(1, di, 1)
        # nrm_emb_c = torch.gather(feat_nrm_seg, 2, choose_emb).contiguous() # nrm_emb_c: [1, 256, 120, 160]
        ...
        # # ###################### semantic segmentation prediction stages #############################
        # rgbd_segs = self.rgbd_seg_layer(rgbd_emb)
        
        
        # ###################### decoding stages for pose estimation #############################
        
        bs, _, p_1, p_2 = ds_pc_emb[-2].shape
        pc_emb_final = ds_pc_emb[-2].view(bs, _, p_1*p_2)
        feat_fuse = torch.cat((feat_nrm_final, pc_emb_final), dim=1)
        bs, feat_dim, _ = feat_fuse.shape
        
        # ###################### pose prediction stages #############################
        self.pose_est = Linear(feat_dim)
        output_r, output_t_est = self.pose_est(feat_fuse)
        ...
        # end_points['pred_rgbd_segs'] = rgbd_segs.contiguous()
        end_points['pred_outputR'] = output_r.contiguous()
        end_points['pred_outputT_est'] = output_t_est.contiguous()
